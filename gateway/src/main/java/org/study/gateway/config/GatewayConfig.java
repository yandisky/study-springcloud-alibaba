package org.study.gateway.config;
...
//        rules.add(this.getGatewayFlowRule("order-gateway"));
    ...
    private void initCustomizedApis() {
        ...
        ApiDefinition api = new ApiDefinition("order_test")
                .setPredicateItems(new HashSet<ApiPredicateItem>() {{
                    add(new ApiPathPredicateItem().setPattern("/server-order/order/test/**").
                            setMatchStrategy(SentinelGatewayConstants.URL_MATCH_STRATEGY_PREFIX));
                }});
        definitions.add(api);
        GatewayApiDefinitionManager.loadApiDefinitions(definitions);
        //在Sentinel管理界面的流控规则中，新增网关流控规则
    }
}
