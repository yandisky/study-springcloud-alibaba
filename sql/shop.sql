-- 创建数据库
create
database if not exists shop default character set utf8 default collate utf8_general_ci;
...
(
    20
) unsigned not null primary key comment '主键，数据id',
    `t_username` varchar
(
    50
...
(
    11
) null default null comment '商品库存'
...
(
    ...
    `t_total_price` decimal
(
    10,
    2
) null default null comment '商品价格（总价）'
...
(
    20
) not null comment '订单id',
    `t_pro_id` bigint
(
    20
) null default null comment '商品id',
    `t_pro_name` varchar
(
    50
) null default null comment '商品名称',
...
(
    10,
    2
) null default null comment '商品价格（单价）',
    `t_number` int
(
    11
) null default null comment '购买数量'
...
(
    ...
    `t_idiom_name` varchar
(
    50
) not null comment '成语名称',
    `t_search_keywords` varchar
(
    16
) not null comment '搜索关键字母，成语的每个字拼音首字母组合',
...
(
    ...
    `t_reserve_column01` varchar
(
    255
) null default null comment '预留字段01',
    `t_reserve_column02` varchar
(
    255
) null default null comment '预留字段02',
    `t_reserve_column03` varchar
(
    255
) null default null comment '预留字段03',
    `t_reserve_column04` varchar
(
    255
) null default null comment '预留字段04'
...
(
    20
) not null comment '成语id',
    `t_sort` tinyint
(
    1
) unsigned not null comment '排序',
    `t_idiom_words` tinytext not null comment '解说文字',
    `t_image_url` varchar
(
    1024
) not null comment '图片文件url',
    `t_audio_url` varchar
(
    1024
) not null comment '语音文件url',
    `t_lrc_url` varchar
(
    1024
) not null comment '歌词文件url',
    `t_create_time` timestamp
(
    0
) not null default current_timestamp comment '创建时间',
...
-- 插入用户表数据
insert into `shop`.`t_user`(`id`, `t_username`, `t_password`, `t_phone`, `t_address`)
values (1001, 'binghe', 'c26be8aaf53b15054896983b43eb6a65', '13212345678', '北京');

-- 插入商品表数据
insert into `shop`.`t_product`(`id`, `t_pro_name`, `t_pro_price`, `t_pro_stock`)
